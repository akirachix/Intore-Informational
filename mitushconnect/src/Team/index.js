 
import './index.css'
const Team= ()=>{
    return (
        <div className='container'>
        <h1 className='teammembers'>Team</h1>
        <div className='team' >

    
        <div>
        <h1 className='teammembers'>Team</h1>
        <div className='team' >
       
           <div className='upperteam'>
            <div>
            <img src="/Images/Aline.jpg" alt="Aline"/>
            <div className='figure'>
 
            <p className="caption" > Aline Mutesi </p>
            <p className="caption">Software Developer</p>
            </div>
            </div>
            <div>
            <img src="/Images/Chemtai.jpeg" alt="Chemtai" className='Chemtai'  />
            <div className='figure'>
            <p className="caption"> Mercy Chemtai</p>
            <p className="caption">Software Developer</p>
            </div>
            </div>
            <div>
            <img src="/Images/Faith.jpeg" alt="Faith" className='Faith'/>
            <div className='figure'>
            <p className="caption"> Faith Wachira</p>
            <p className="caption">Software Developer</p>
            </div>
            </div>
            </div>
            <p> Aline Mutesi </p>
            <p>Software Developer</p>
            </div>
            </div>
         
            <div>
            <img src="/Images/Chemtai.jpeg" alt="Chemtai" className='Chemtai'  />
            <div className='figure'>
            <p> Mercy Chemtai</p>
            <p>Software Developer</p>
            </div>
            </div>
            
            <div>
            <img src="/Images/Faith.jpeg" alt="Faith" className='Faith'/>
            <div className='figure'>
            <p> Faith Wachira</p>
            <p>Software Developer</p>
            </div>
            </div>
            </div>

            


            <div className='buttomteam'>
            <div>
            <img src="/Images/Patricia.jpeg" alt="Patricia" className='patricia'/>
            <div className='figure'>
 
            <p className="caption"> Patricia Ainembabazi</p>
            <p className="caption">Software Developer</p>
            </div>
           </div>
           <div>
            <img src="/Images/Raziah.jpeg" alt="Raziah"  className='raziah'/>
            <div className='figure'>
            <p className="caption"> Raziah Abdullahi</p>
            <p className="caption">Software Developer</p>
            </div>
            </div>
            </div>
            </div>
            
    )
}
export default Team

           
